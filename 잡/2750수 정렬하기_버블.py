...
def bubble_sort(a: MutableSequence) -> None:
    n = len(a)
    for i in range(n):    # 0부터n-1까지 i 값에 차례대로 반복
...
if __name__ == '__main__':
    ...
    for i in range(num):        # i 라는 변수에 0부터 num-1만큼 대입
        x[i] = int(input())        # 대입한 i 변수를 x[] 리스트에 넣음.
        
    bubble_sort(x)      # 배열 x를 버블 정렬 시킨다.

    for i in range(num):    # i라는 변수에 num을 대입
        print(x[i])   # print(인덱스 '=' 인덱스에 해당하는 값) 형식으로 출력 ex) num 이 3이고 리스트 x가 [10, 20, 30]인 경우 print(1, x[1]) -> 1 = 20